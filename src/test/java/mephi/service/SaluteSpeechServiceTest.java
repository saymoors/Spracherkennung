...
class SaluteSpeechServiceTest {
    ...
    void startRecognitionUploadsFileAndCreatesSberTask() throws Exception {
        ...
        when(sberFileClient.uploadFile("uploads/lecture.mp3", AudioFormat.MP3, "access-token"))
                .thenReturn(new SberUploadFileResponse(requestFileId, 200));
        when(dataService.getAudioLanguage(1)).thenReturn("ru-RU");
        when(dataService.getAudioFormat(1)).thenReturn(AudioFormat.MP3);
        when(sberTaskClient.createTask(requestFileId, "ru-RU", AudioFormat.MP3, "access-token"))
